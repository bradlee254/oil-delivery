...
<template>
  <div class="min-h-screen bg-slate-100 py-8 px-6">
    <div class="max-w-4xl mx-auto">
      ...
      <!-- Fuel Request Form -->
      <div
        class="mb-8 bg-white rounded-xl shadow-md overflow-hidden border-l-4 border-amber-500"
      >
        <div class="bg-slate-900 px-6 py-4">
          <h2 class="text-xl font-bold text-white flex items-center">
            <svg
              class="w-6 h-6 mr-2 text-amber-500"
              fill="currentColor"
              viewBox="0 0 20 20"
            >
              <path
        ...
        <form @submit.prevent="submitRequest" class="p-6 space-y-5">
          ...
          <div>
            ...
            <p class="text-xs text-slate-500 mt-1">
              Enter longitude and latitude separated by comma
            </p>
          </div>
      ...
      <!-- List of Requests -->
      <div
        class="bg-white rounded-xl shadow-md overflow-hidden border-l-4 border-indigo-600"
      >
        <div class="bg-slate-900 px-6 py-4">
          <h2 class="text-xl font-bold text-white flex items-center">
            <svg
        ...
        <div class="p-6">
              ...
              <div class="flex justify-between items-start">
                <div class="flex-1">
                  <div class="flex items-center mb-2">
                    <span
                      class="bg-slate-900 text-white px-3 py-1 rounded-full text-xs font-bold mr-2"
                    >
                      {{ req.fuelType }}
                    </span>
                    <span
                      class="bg-amber-500 text-slate-900 px-3 py-1 rounded-full text-xs font-bold"
                    >
                      {{ req.amount }}L
                    </span>
                  </div>
          ...
          <div v-if="fuelStore.requests.length === 0" class="text-center py-12">
            <svg
            ...
            <p class="text-slate-400 text-sm">
              Create your first fuel request above!
            </p>
          </div>
        </div>
      </div>
